...
for (i in 1:length(var_list)) {
  ...
  model_list[[i]] <- lm(model_formula, data = clean_target2)
  coefs_model <- broom.mixed::tidy(model_list[[i]]) %>% 
    mutate(variable = var_list[i]) 
  lmer_df <- bind_rows(lmer_df, coefs_model)
  
  null_formula <- as.formula(paste(var_list[i], "~ 1"))
  null_list[[i]] <- lm(null_formula, data = clean_target2)
...
write.csv(aic_df, "02_clean_data/61_03_pp_lmer_aics.csv")
write.xlsx(aic_df, "02_clean_data/61_03_pp_lmer_aics.xlsx")