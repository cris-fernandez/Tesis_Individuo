...
for (i in 1:length(var_list)) {
  ...
  model_list_aa[[i]] <- mgcv::gam(model_formula, data = aa_target, 
                                  method= "ML")
  ci <- summary(emmeans(model_list_aa[[i]], ~ vigor_id))
...
for (i in 1:length(var_list)) {
  ...
  model_list_ps[[i]] <- mgcv::gam(model_formula, data = ps_target, 
                                  method= "ML")
  ci <- summary(emmeans(model_list_ps[[i]], ~ vigor_id))
  ci$variable <- var_list[i]
...
for (i in 1:length(var_list)) {
  ...
  model_list_pp[[i]] <- mgcv::gam(model_formula, data = pp_target, 
                                  method= "ML")
  ci <- summary(emmeans(model_list_pp[[i]], ~ vigor_id))
  ci$variable <- var_list[i]
...
confint_aa$significant <- ifelse(confint_aa$variable %in% c("height", "age"), "yes", "no")

fig_aa <- ggplot(confint_aa) + 
  geom_point(aes(x = variable, y = emmean, colour = vigor_id,
...
confint_pp$significant <- ifelse(confint_ps$variable %in% 
                                   c("total_chl_fw_22", "chlor_a_fw_22", 
                                     "chlor_b_fw_22", "chl_xc_22"), "yes", "no")
...
tiff("04_figures/62_02_MegaFig_d3.tiff", units = "mm", width = 640, height = 320,
