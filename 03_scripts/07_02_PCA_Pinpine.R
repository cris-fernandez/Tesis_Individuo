...
# 5.- Filtering species and status ####
...
# The first two components explain only 43.05% of the data variance!
...
biplot_all <- ggplot() +
  ...
  scale_fill_gradientn(colours = c(scales::alpha("#db5f02", 0),
                                   scales::alpha("#db5f02", 0.4))) +
  geom_contour(data = dens_df,
               ...
               size = 0.7,
               linetype = "dashed") + 
  geom_point(data = pca_df, aes(x = Comp.1, y = Comp.2), color = "#db5f02", 
             alpha = 0.6) +
...
cold_target <- cold_target %>% 
  ...
  dplyr::select(c(mean_def_obs, height, dbh, chl_fw_22, xc_fw_22, chla_chlb_22,
                  ...
                  mean_10, mean_05, Rt12, Rt17, Rt22, Rs12, Rs17, 
                  tree_number, sp_id, spot_status)) %>% 
  dplyr::select(sort(names(.))) %>% na.omit()
...
norm_cold <- cold_target %>%
  mutate(defoliation_ST = (mean_def_obs - mean(mean_def_obs, na.rm = T)) / sd(mean_def_obs, na.rm = T),
         ...
         chl_ab_ST = (chla_chlb_22 - mean(chla_chlb_22, na.rm = T)) / sd(chla_chlb_22, na.rm = T),
         chl_xc_ST = (chl_xc_22 - mean(chl_xc_22, na.rm = T)) / sd(chl_xc_22, na.rm = T),
         percent_c_ST = (percent_c - mean(percent_c, na.rm = T)) / sd(percent_c, na.rm = T),         percent_n_ST = (percent_n - mean(percent_n, na.rm = T)) / sd(percent_n, na.rm = T),
         cn_ratio_ST = (cn_ratio - mean(cn_ratio, na.rm = T)) / sd(cn_ratio, na.rm = T),
         leaf_d13c_ST = (leaf_d13c - mean(leaf_d13c, na.rm = T)) / sd(leaf_d13c, na.rm = T),
         leaf_d15n_ST = (leaf_d15n - mean(leaf_d15n, na.rm = T)) / sd(leaf_d15n, na.rm = T),
         leaf_d18o_ST = (leaf_d18o - mean(leaf_d18o, na.rm = T)) / sd(leaf_d18o, na.rm = T),
...
norm_cold <- norm_cold %>% dplyr::select(contains("_ST")) %>% 
  dplyr::select(-spot_status)
norm_hot <- norm_hot %>% dplyr::select(contains("_ST")) %>% 
  dplyr::select(-spot_status)

# PCA analyses
pca_cold <- prcomp(norm_cold)
summary(pca_cold)

pca_hot <- prcomp(norm_hot)
summary(pca_hot)

cold_pca_df <- cbind(norm_cold, pca_cold$x) # Scores for the points
cold_pca_df$sp_id <- cold_target$sp_id
cold_pca_df$spot_status <- cold_target$spot_status

hot_pca_df <- cbind(norm_hot, pca_hot$x) # Scores for the points
hot_pca_df$sp_id <- hot_target$sp_id
hot_pca_df$spot_status <- hot_target$spot_status

# Biplot
fviz_pca_var(pca_cold, col.var = "black")
fviz_pca_var(pca_hot, col.var = "black")


# 13.- Biplot - spot status ####

## 11.1.- Loadings dataframe ####

cold_loadings_df <- as.data.frame(pca_cold$rotation[, 1:2])
hot_loadings_df <- as.data.frame(pca_hot$rotation[, 1:2])

cold_loadings_df$variable <- rownames(cold_loadings_df)
hot_loadings_df$variable <- rownames(hot_loadings_df)

cold_loadings_df$varnames <- loadings_df$varnames
hot_loadings_df$varnames <- loadings_df$varnames

## 11.2.- Multiplying ####

cold_loadings_df <- cold_loadings_df %>% 
  mutate(Comp.1 = scale_factor * PC1,
         Comp.2 = scale_factor * PC2)
hot_loadings_df <- hot_loadings_df %>% 
  mutate(Comp.1 = scale_factor * PC1,
         Comp.2 = scale_factor * PC2)

## 11.3.- Density raster ####
cold_dens <- kde2d(x = cold_pca_df$PC1,
                   y = cold_pca_df$PC2,
                   n = 200)  # resolution
hot_dens <- kde2d(x = hot_pca_df$PC1,
                  y = hot_pca_df$PC2,
                  n = 200)  # resolution

cold_dens_df <- as.data.frame(expand.grid(x = cold_dens$x, y = cold_dens$y))
cold_dens_df$z <- as.vector(cold_dens$z)
hot_dens_df <- as.data.frame(expand.grid(x = hot_dens$x, y = hot_dens$y))
hot_dens_df$z <- as.vector(hot_dens$z)

## 11.4.- Density 50% ####
cold_z_sorted <- sort(as.vector(cold_dens$z))
cold_cdf <- cumsum(cold_z_sorted) / sum(cold_z_sorted)
cold_50 <- cold_z_sorted[which.min(abs(cold_cdf - 0.5))]

hot_z_sorted <- sort(as.vector(hot_dens$z))
hot_cdf <- cumsum(hot_z_sorted) / sum(hot_z_sorted)
hot_50 <- hot_z_sorted[which.min(abs(hot_cdf - 0.5))]

## 11.5.- Biplot, cold ####

biplot_cold <- ggplot() +
  geom_raster(data = cold_dens_df, aes(x = x, y = y, fill = z), interpolate = TRUE) +
  scale_fill_gradientn(colours = c(scales::alpha("navy", 0),
                                   scales::alpha("navy", 0.4))) +
  geom_contour(data = cold_dens_df,
               aes(x = x, y = y, z = z),
               breaks = level_50,
               color = "grey40",
               size = 0.7,
               linetype = "dashed") + 
  geom_point(data = pca_cold, aes(x = PC1, y = PC2), color = "navy", 
             alpha = 0.6) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey40") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey40") +
  geom_segment(data = cold_loadings_df,
               aes(x = 0, y = 0, xend = Comp.1, yend = Comp.2),
               arrow = arrow(length = unit(0.2, "cm")),
               color = "black", size = 0.8) +
  guides(fill = "none") +
  ...
  theme(axis.text.x = element_text(size = 15),
        ...
        panel.border = element_rect(color = "black", 
                                    fill = NA, 
                                    linewidth = 0.5)) + 
  geom_text(data = cold_loadings_df,
            aes(x = Comp.1 * 1.1, y = Comp.2 * 1.1, label = varnames),
...
biplot_hot <- ggplot() +
  geom_raster(data = hot_dens_df, aes(x = x, y = y, fill = z), interpolate = TRUE) +
  scale_fill_gradientn(colours = c(scales::alpha("red4", 0),
                                   scales::alpha("red4", 0.4))) +
  geom_contour(data = hot_dens_df,
               aes(x = x, y = y, z = z),
               breaks = level_50,
               color = "grey40",
               size = 0.7,
               linetype = "dashed") + 
  geom_point(data = pca_hot, aes(x = PC1, y = PC2), color = "red4", 
             alpha = 0.6) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey40") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey40") +
  geom_segment(data = hot_loadings_df,
               aes(x = 0, y = 0, xend = Comp.1, yend = Comp.2),
               arrow = arrow(length = unit(0.2, "cm")),
               color = "black", size = 0.8) +
  guides(fill = "none") +
  ...
  theme(axis.text.x = element_text(size = 15),
        ...
        panel.border = element_rect(color = "black", 
                                    fill = NA, 
                                    linewidth = 0.5)) + 
  geom_text(data = hot_loadings_df,
            aes(x = Comp.1 * 1.1, y = Comp.2 * 1.1, label = varnames),
            size = 2.5, fontface = "bold")

# 14.- Plotting ####

tiff("04_figures/07_02_pca_pinpine.tiff", units = "mm", 
     width = 400, height = 400,
     res = 700, compression = "lzw")
biplot_all / (biplot_cold + biplot_hot) + 
