...
ci_df_3 <- read.csv("02_clean_data/63_02_AICc_defo.csv", 
                    header = T, sep = ",") %>% dplyr::select(-X)
...
fig_2_h <- ggplot(h_df) + 
  ...
  scale_color_manual(breaks = c("coldspot", "hotspot"),
                     ...
                     labels = c("Non-declining",
                                "Declining"),
                     name = "") + 
  scale_alpha_manual(limits = c("yes", "no"),
                     values = c("yes" = 1,
                                "no" = 0.3),
                     name = "",
...
sla_df$significant <- factor(sla_df$significant, levels = c("yes", "no"))
...
n_df$significant <- factor(n_df$significant, levels = c("yes", "no"))
...
chl_df$significant <- factor(chl_df$significant, levels = c("yes", "no"))
...
xc_df$significant <- factor(xc_df$significant, levels = c("yes", "no"))
...
d13c_df$significant <- factor(d13c_df$significant, levels = c("yes", "no"))
...
d18o_df$significant <- factor(d18o_df$significant, levels = c("yes", "no"))
...
bai80_df$significant <- factor(bai80_df$significant, levels = c("yes", "no"))
...
fig_3_sla <- ggplot(hot_target) + 
  ...
  geom_smooth(data = hot_target %>% filter(sp_id != "Pinsylv"),
              aes(x = mean_def_obs, y = sla_22, colour = sp_id, fill = sp_id), 
              linewidth = 2, alpha = 0.3, method = "lm") +
  scale_shape_manual(values = c(16,16,16,15,15,17,18)) + 
  scale_color_manual(breaks = c("Abialba", "Pinsylv", "Pinpine"),
                     values = c("Abialba" = "#785EF0",
...
fig_3_d13c <- ggplot(hot_target) + 
  ...
  geom_smooth(data = hot_target %>% filter(sp_id == "Pinpine"),
              aes(x = mean_def_obs, y = leaf_d13c, colour = sp_id, fill = sp_id), 
              linewidth = 2, alpha = 0.3, method = "lm") +
  scale_shape_manual(values = c(16,16,16,15,15,17,18)) + 
  scale_color_manual(breaks = c("Abialba", "Pinsylv", "Pinpine"),
                     values = c("Abialba" = "#785EF0",
...
fig_3_bai80 <- ggplot(hot_target) + 
  ...
  geom_point(aes(x = mean_def_obs, y = mean_1980, colour = sp_id,
                 shape = pair_id), size = 3.05, alpha = 0.4) + 
  geom_smooth(data = hot_target %>% filter(sp_id == "Pinpine"),
              aes(x = mean_def_obs, y = mean_1980, colour = sp_id, fill = sp_id), 
              linewidth = 2, alpha = 0.3, method = "lm") +
  scale_shape_manual(values = c(16,16,16,15,15,17,18)) + 
  scale_color_manual(breaks = c("Abialba", "Pinsylv", "Pinpine"),
                     values = c("Abialba" = "#785EF0",
...
# tiff("04_figures/63_11_Fig1_fixed_V8.tiff",
...
#   plot_layout(widths = c(3.2, -0.35, 3.2, 0.01, 3.2, -0.35, 3.2))
# 
# 
# final_plot
# 
...
tiff("04_figures/63_12_Fig1_lms_v1.tiff",
     units = "mm", width = 600, height = 550,
