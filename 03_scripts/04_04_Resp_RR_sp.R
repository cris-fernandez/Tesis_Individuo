...
rr_target <- clean_target %>% 
  ...
  summarise(mean_wc = mean(wc_22, na.rm = T),
            ...
            mean_chl_ab = mean(chla_chlb_22, na.rm = T),
            mean_xc = mean(xc_fw_22, na.rm = T),
            mean_chl_xc = mean(chl_xc_22, na.rm = T),
...
varnames <- c("EWT", "Leaves δ15N", "Chl.", "Leaves N content", "Chl. a", 
              "Carotenoids", "Leaves WC", "Chl. a / Chl. b ratio", "Chl. b", 
              "Leaves C:N ratio", "Wood δ13C 2017", "Leaves C content", 
              "SLA", "Wood δ13C 2022", "Leaves δ18O", 
              "Chl. / Carotenoids ratio", "Leaves δ13C") %>% rev()
...
tiff("04_figures/04_04_Resp_response_ratios_sp.tiff", units = "mm", 
