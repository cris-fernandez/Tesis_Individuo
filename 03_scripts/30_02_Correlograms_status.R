...
# Filtering hot and coldspots separately
...
cold_target <- na.omit(cold_target)
hot_target <- na.omit(hot_target)

# Correlogram 

correlogram_cold <- cor(cold_target)
correlogram_hot <- cor(hot_target)

# P-value matrix creation

cold_matrix <- cor_pmat(cold_target)
hot_matrix <- cor_pmat(hot_target)

correlogram_cold <- ggcorrplot(correlogram_cold, 
                          type = "lower",
                          lab = TRUE,
                          method = "circle", 
                          p.mat = cold_matrix, 
...
sum(cold_matrix < 0.05) / 2 #155
sum(hot_matrix < 0.05) / 2 #179
