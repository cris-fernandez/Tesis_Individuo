...
pca_df_aa$def_gradient <- ifelse(pca_df_aa$spot_status == "coldspot", NA, pca_df_aa$mean_def_obs)
pca_df_ps$def_gradient <- ifelse(pca_df_ps$spot_status == "coldspot", NA, pca_df_ps$mean_def_obs)
pca_df_pp$def_gradient <- ifelse(pca_df_pp$spot_status == "coldspot", NA, pca_df_pp$mean_def_obs)

# Now I also need to know the max. and min. levels of defoliation so all 
# scales are constant for the three species

min_def <- min(clean_target$mean_def_obs)
max_def <- max(clean_target$mean_def_obs)
...
tiff("04_figures/51_08_PCA_ellipses_gradient_onlyred.tiff", units = "mm",
