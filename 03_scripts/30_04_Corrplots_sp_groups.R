...
### 6.1.4.- Photosynthesis pigments ####

aa_photosynthesis <- aa_target %>% 
  dplyr::select(c("Chl.", "Chl. a / b", "Chl. a", "Chl. b")) %>% 
  cor()
aa_photosynthesis_plot <- corrplot::corrplot.mixed(aa_photosynthesis, upper = "ellipse")

### 6.1.5.- Photoprotection pigments ####

aa_photoprotection <- aa_target %>% 
  dplyr::select(c("Car.", "Chl. / Car.")) %>% 
  cor()
aa_photoprotection_plot <- corrplot::corrplot.mixed(aa_photoprotection, upper = "ellipse")
...
corrplot::corrplot.mixed(aa_photoprotection, upper = "ellipse", main = "Photoprotection")
...
### 6.2.5.- Photoprotection pigments ####

ps_photoprotection <- ps_target %>% 
  dplyr::select(c("Car.", "Chl. / Car.")) %>% 
  cor()
ps_photoprotection_plot <- corrplot::corrplot.mixed(ps_photoprotection, upper = "ellipse")
...
corrplot::corrplot.mixed(ps_photoprotection, upper = "ellipse", main = "Photoprotection")
...
pp_photoprotection <- pp_target %>% 
  dplyr::select(c("Car.", "Chl. / Car.")) %>% 
  cor()
pp_photoprotection_plot <- corrplot::corrplot.mixed(pp_photoprotection, upper = "ellipse")
...
corrplot::corrplot.mixed(pp_photoprotection, upper = "ellipse", main = "Photoprotection")
