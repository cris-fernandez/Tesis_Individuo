...
leaf_d18o_corrected ~ height + mean_1980 + sla_22
'
# 8.- Constrained model ####
...
# This model has a p = 0.016 (non-significant), so the model does not adjust well
# to my data
...
# p = 0.000 --> the model adjusts to my data, so there is a model in between I can try
...
liber_sem <- sem(sem_model,
                 ...
                 group.partial = c("leaf_d18o_corrected ~ mean_1980"),
                 fixed.x = F)
