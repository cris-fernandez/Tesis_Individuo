...
model2_all <- read.csv("02_clean_data/45_01_models_2way_continuous.csv") %>% 
  dplyr::select(c(variable, p_val_hot)) %>% 
  rename(p_all = p_val_hot)

model2_aa <- read.csv("02_clean_data/45_02_models_2way_continuous_aa.csv") %>% 
  dplyr::select(c(variable, p_val_hot)) %>% 
  rename(p_aa = p_val_hot)

model2_ps <- read.csv("02_clean_data/45_03_models_2way_continuous_ps.csv") %>% 
  dplyr::select(c(variable, p_val_hot)) %>% 
  rename(p_ps = p_val_hot)

model2_pp <- read.csv("02_clean_data/45_04_models_2way_continuous_pp.csv") %>% 
  dplyr::select(c(variable, p_val_hot)) %>% 
  rename(p_pp = p_val_hot)

# 2.- Reading all model3 data ####

model3_all <- read.csv("02_clean_data/45_05_models_3way_continuous.csv") %>% 
  dplyr::select(c(variable, p_val_damaged)) %>% 
  rename(p_all = p_val_damaged)

model3_aa <- read.csv("02_clean_data/45_06_models_3way_continuous_aa.csv") %>% 
  dplyr::select(c(variable, p_val_damaged)) %>% 
  rename(p_aa = p_val_damaged)

model3_ps <- read.csv("02_clean_data/45_07_models_3way_continuous_ps.csv") %>% 
  dplyr::select(c(variable, p_val_damaged)) %>% 
  rename(p_ps = p_val_damaged)

model3_pp <- read.csv("02_clean_data/45_08_models_3way_continuous_pp.csv") %>% 
  dplyr::select(c(variable, p_val_damaged)) %>% 
...
write.xlsx(model2_df, "02_clean_data/45_09_models2_excel.xlsx")
write.xlsx(model3_df, "02_clean_data/45_09_models3_excel.xlsx")