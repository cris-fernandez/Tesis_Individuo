...
focus_map <- ggplot() +
  ...
  scale_y_continuous(breaks = seq(40, 43, 1)) + 
  ...
  ggspatial::annotation_north_arrow(
    ...
    pad_x = unit(6, "mm"), pad_y = unit(8, "mm"),
    height = unit(3.3, "cm"),
    width = unit(3.3, "cm"),
...
# 10.- Plotting Europe map ####

distrib_world <- ggplot() +
  geom_sf(data = mundo, fill = "#d3d3d3", alpha = 0.6, col = "black", linewidth = 0.6) +  
  geom_sf(data = abialba_world, aes(fill = "Abies alba"), col = NA, alpha = 0.70) +
  geom_sf(data = pinsylv_world, aes(fill = "Pinus sylvestris"), col = NA, alpha = 0.52) +
  ...
  ggspatial::annotation_north_arrow(
    location = "tr", which_north = "true",
    pad_x = unit(0.1, "mm"), pad_y = unit(0.8, "mm"),
    style = ggspatial::north_arrow_fancy_orienteering(
      fill = c("black", "white"),
      line_col = "#565656",
      text_family = "sans"))
...
tiff("04_figures/05_02_fig1_v4.tiff", units = "mm", width = 310, height = 300,
     res = 600, compression = "lzw")
final_map2
dev.off()