...
aa_target <- clean_target %>% filter(sp_id == "Abialba" & spot_status == "hotspot") %>% 
  dplyr::select(-c(sp_id, spot_status))
ps_target <- clean_target %>% filter(sp_id == "Pinsylv" & spot_status == "hotspot") %>% 
  dplyr::select(-c(sp_id, spot_status))
pp_target <- clean_target %>% filter(sp_id == "Pinpine" & spot_status == "hotspot") %>% 
  dplyr::select(-c(sp_id, spot_status))
...
tiff("04_figures/30_08_correlogram_aa_hot.tiff", units = "mm", 
     width = 200, height = 200,
     res = 700, compression = "lzw")
correlogram_aa2
dev.off()

tiff("04_figures/30_08_correlogram_ps_hot.tiff", units = "mm", 
     width = 200, height = 200,
     res = 700, compression = "lzw")
correlogram_ps2
dev.off()

tiff("04_figures/30_08_correlogram_pp_hot.tiff", units = "mm", 
     width = 200, height = 200,
     res = 700, compression = "lzw")
correlogram_pp2
dev.off()
