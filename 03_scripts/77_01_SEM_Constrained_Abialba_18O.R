...
# This model has a p = 0.185 (significant), so the model does not adjust well
# to my data
...
liber_sem <- sem(sem_model,
                 ...
                 group.partial = c("leaf_d18o_corrected ~ sla_22"),
                 fixed.x = F)

summary(liber_sem, standardized = TRUE, fit.measures = TRUE)
# p-val = 0.156, df = 10, chi-sq = 25.086 --> Still need to liberate more paths
