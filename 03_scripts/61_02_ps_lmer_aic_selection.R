...
clean_target$pair_id <- as.factor(clean_target$pair_id)
...
for (i in 1:length(var_list)) {
  model_formula <- as.formula(paste(var_list[i], 
                                    "~ mean_def_obs + (1|pair_id)"))
  
  clean_target2 <- clean_target %>% filter(!is.na(var_list[i]))
  model_list[[i]] <- lmer(model_formula, data = clean_target2, REML = F)
  coefs <- broom.mixed::tidy(model_list[[i]], effects = "fixed")%>% 
    mutate(variable = var_list[i])
  lmer_df <- bind_rows(lmer_df, coefs)
  
  null_formula <- as.formula(paste(var_list[i], "~ 1 + (1|pair_id)"))
  null_list[[i]] <- lmer(null_formula, data = clean_target2, REML = F)
  coefs_null <- broom.mixed::tidy(null_list[[i]]) %>% 
    mutate(variable = var_list[i])
...
for (i in 1:length(var_list)) {
  aic_df[i, 1] <- AICc(model_list[[i]], null_list[[i]])[1,2] # Model list
  aic_df[i, 2] <- AICc(model_list[[i]], null_list[[i]])[2,2] # Null list
  print(i)
}

colnames(aic_df) <- c("full_model", "null_model")
aic_df$variable <- var_list
aic_df$delta_aicc <- aic_df$full_model - aic_df$null_model

write.csv(aic_df, "02_clean_data/61_02_ps_lmer_aics.csv")
write.xlsx(aic_df, "02_clean_data/61_02_ps_lmer_aics.xlsx")