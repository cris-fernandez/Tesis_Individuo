...
aa_target <- clean_target %>% filter(sp_id == "Abialba") %>% mutate(mean_def_obs = log1p(mean_def_obs))
ps_target <- clean_target %>% filter(sp_id == "Pinsylv") %>% mutate(mean_def_obs = log1p(mean_def_obs))
pp_target <- clean_target %>% filter(sp_id == "Pinpine") %>% mutate(mean_def_obs = log1p(mean_def_obs))
...
tiff("04_figures/44_05_Scatters_logx_aa.tiff", units = "mm", width = 1200, height = 600,
...
tiff("04_figures/44_05_Scatters_logx_ps.tiff", units = "mm", width = 1200, height = 600,
     res = 400, compression = "lzw")
...
tiff("04_figures/44_05_Scatters_logx_pp.tiff", units = "mm", width = 1200, height = 600,
     res = 400, compression = "lzw")
