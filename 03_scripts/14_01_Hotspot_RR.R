...
rownames(rr_target) <- rr_target$vigor_id
rr_target <- rr_target %>% 
  t() %>% 
  as.data.frame()

rownames(sd_target) <- sd_target$vigor_id
sd_target <- sd_target %>% 
  t() %>% 
  as.data.frame
...
rr_target <- rr_target[-which(rownames(rr_target) == "vigor_id"), ]
sd_target <- sd_target[-which(rownames(sd_target) == "vigor_id"), ]
...
colnames(rr_df) <- c("mean_hot_healthy", "mean_hot_damaged", "var", "se_hot_healthy", "se_hot_damaged")
...
rr_df <- rr_df %>% 
  mutate_at(vars(mean_hot_healthy, mean_hot_damaged, 
                 se_hot_damaged, se_hot_healthy), as.numeric)

rr_df$response_ratio <- abs(log(rr_df$mean_hot_damaged / rr_df$mean_hot_healthy))
...
varnames <- c("BAI 10 years", "BAI 05 years", "BAI 15 years", "BAI 20 years", 
              "Leaf δ15N", "BAI since 1980", "Rs 2012", "Rs 2017", "Hegyi Index", 
              "Age", "N content", "BAI", "Rt 2022", "Leaf C:N", "Rt 2012", 
              "Rt 2017", "Chlorophylls content", "SLA", "Carotenoids content", 
              "d.b.h.", "Leaf δ13C", "Chl a / Chl b", "Chl / carotenoids", 
              "Leaf δ18O", "C content", "Wood δ13C 2022", "Height", "Wood δ13C 2017") %>% rev()
...
tiff("04_figures/14_01_ranked_response_ratios.tiff", units = "mm", 
