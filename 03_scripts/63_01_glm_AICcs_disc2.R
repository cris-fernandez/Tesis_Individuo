...
# Transforming spot status and pair_id into a factor so they can be modellised:
...
for (i in 1:length(var_list)) {
  model_formula <- as.formula(paste(var_list[i], "~ spot_status + (1|pair_id)"))
  ps_target2 <- ps_target %>% filter(!is.na(var_list[i]))
  model_list_ps[[i]] <- lmer(model_formula, data = ps_target2, REML = F)
  ...
  null_list_ps[[i]] <- lmer(null_formula, data = ps_target2, REML = F)
...
write.csv(ci_df, "02_clean_data/63_01_AICc_discrete2_glm.csv")
