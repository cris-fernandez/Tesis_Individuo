...
rownames(rr_target) <- paste0(rr_target$vigor_id, "_", 
                              rr_target$sp_id)
rr_target <- rr_target %>% 
  t() %>% 
  as.data.frame()

rownames(sd_target) <- paste0(sd_target$vigor_id, "_", 
                              sd_target$sp_id)
sd_target <- sd_target %>% 
  t() %>% 
  as.data.frame
...
rr_target <- rr_target[-c(which(rownames(rr_target) == "vigor_id"),
                          which(rownames(rr_target) == "sp_id")), ]
sd_target <- sd_target[-c(which(rownames(sd_target) == "vigor_id"),
...
colnames(rr_df) <- c("mean_hot_healthy_Abialba", "mean_hot_healthy_Pinsylv", "mean_hot_healthy_Pinpine",
                     "mean_hot_damaged_Abialba", "mean_hot_damaged_Pinsylv", "mean_hot_damaged_Pinpine",
                     "var", 
                     "se_hot_healthy_Abialba", "se_hot_healthy_Pinsylv", "se_hot_healthy_Pinpine",
                     "se_hot_damaged_Abialba", "se_hot_damaged_Pinsylv", "se_hot_damaged_Pinpine")

rr_df2 <- rr_df %>%
  pivot_longer(cols = -var, names_to = c("stat", "vigor_id", "sp_id"),
               names_pattern = "(mean|se)_(hot_healthy|hot_damaged)_(.*)") %>% 
  mutate(var_status = paste0(stat, "_", vigor_id)) %>% 
  dplyr::select(-c(stat, vigor_id)) %>% 
  pivot_wider(names_from = "var_status",
              values_from = "value") %>% 
  mutate_at(vars(mean_hot_healthy, mean_hot_damaged, se_hot_healthy, se_hot_damaged), as.numeric)

# 7.- Calculating the log response ratio ####

rr_df2$response_ratio <- abs(log(rr_df2$mean_hot_damaged / rr_df2$mean_hot_healthy))
...
rr_df2 <- rr_df2 %>% 
  mutate(se_rr = sqrt((se_hot_damaged / mean_hot_damaged)^2 + (se_hot_healthy / mean_hot_healthy)^2))
...
tiff("04_figures/34_01_ranked_rr_morpho.tiff", units = "mm", 
...
tiff("04_figures/34_01_ranked_rr_physio.tiff", units = "mm", 
     width = 200, height = 200,
     res = 700, compression = "lzw")
physio_rr_plot
dev.off()
...
tiff("04_figures/34_01_ranked_rr_whole.tiff", units = "mm", 
     width = 200, height = 200,
     res = 700, compression = "lzw")
whole_rr_plot
dev.off()
...
tiff("04_figures/34_01_ranked_rr_panels.tiff", units = "mm", 
