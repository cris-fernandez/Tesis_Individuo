...
biplot_all <- ggplot() +
  ...
  scale_fill_gradientn(colours = c(scales::alpha("#1b9e77", 0),
                                   scales::alpha("#1b9e77", 0.4))) +
  geom_contour(data = dens_df,
               ...
               size = 0.7,
               linetype = "dashed") + 
  geom_point(data = pca_df, aes(x = Comp.1, y = Comp.2), color = "#1b9e77", 
             alpha = 0.6) +
...
# PCA analyses
pca_cold <- princomp(norm_cold)
summary(pca_cold)

pca_hot <- princomp(norm_hot)
summary(pca_hot)

cold_pca_df <- cbind(norm_cold, pca_cold$scores) # Scores for the points
cold_pca_df$sp_id <- cold_target$sp_id
cold_pca_df$spot_status <- cold_target$spot_status

hot_pca_df <- cbind(norm_hot, pca_hot$scores) # Scores for the points
...
cold_loadings_df <- as.data.frame(pca_cold$loadings[, 1:2])
hot_loadings_df <- as.data.frame(pca_hot$loadings[, 1:2])
...
## 11.3.- Density raster ####
cold_dens <- kde2d(x = cold_pca_df$Comp.1,
                   y = cold_pca_df$Comp.2,
                   n = 200)  # resolution
hot_dens <- kde2d(x = hot_pca_df$Comp.1,
                   y = hot_pca_df$Comp.2,
                   n = 200)  # resolution
...
biplot_cold <- ggplot() +
  ...
  geom_contour(data = cold_dens_df,
               ...
               size = 0.7,
               linetype = "dashed") + 
  geom_point(data = pca_cold, aes(x = Comp.1, y = Comp.2), color = "navy", 
             alpha = 0.6) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey40") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey40") +
...
biplot_hot <- ggplot() +
  ...
  geom_contour(data = hot_dens_df,
               ...
               size = 0.7,
               linetype = "dashed") + 
  geom_point(data = pca_hot, aes(x = Comp.1, y = Comp.2), color = "red4", 
             alpha = 0.6) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey40") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey40") +
...
tiff("04_figures/07_02_pca_pinsylv.tiff", units = "mm", 
