...
pca_ch <- pca_df %>% filter(vigor_id == "cold_healthy")
dens_ch <- kde2d(x = pca_ch$Comp.1,
                 y = pca_ch$Comp.2,
                 n = 200) 
pca_hh <- pca_df %>% filter(vigor_id == "hot_healthy")
dens_hh <- kde2d(x = pca_hh$Comp.1,
              y = pca_hh$Comp.2,
              n = 200) 

pca_hd <- pca_df %>% filter(vigor_id == "hot_damaged")
dens_hd <- kde2d(x = pca_hd$Comp.1,
                 y = pca_hd$Comp.2,
                 n = 200) 

dens_df <- as.data.frame(expand.grid(x = dens$x, y = dens$y))
densdf_ch <- as.data.frame(expand.grid(x = dens_ch$x, y = dens_ch$y))
densdf_hh <- as.data.frame(expand.grid(x = dens_hh$x, y = dens_hh$y))
densdf_hd <- as.data.frame(expand.grid(x = dens_hd$x, y = dens_hd$y))

dens_df$z <- as.vector(dens$z)
densdf_ch$z <- as.vector(dens_ch$z)
densdf_hh$z <- as.vector(dens_hh$z)
densdf_hd$z <- as.vector(dens_hd$z)
...
z_ch <- sort(as.vector(dens_ch$z))
cdf <- cumsum(z_ch) / sum(z_ch)
level_ch <- z_ch[which.min(abs(cdf - 0.5))]

z_hh <- sort(as.vector(dens_hh$z))
cdf <- cumsum(z_hh) / sum(z_hh)
level_hh <- z_hh[which.min(abs(cdf - 0.5))]

z_hd <- sort(as.vector(dens_hd$z))
cdf <- cumsum(z_hh) / sum(z_hd)
level_hd <- z_hd[which.min(abs(cdf - 0.5))]

####### PRUEBA #######
ggplot() +
  geom_raster(data = densdf_ch, aes(x = x, y = y, fill = z), interpolate = TRUE) +
  scale_fill_gradientn(colours = c(scales::alpha("#2274A5", 0),
                                   scales::alpha("#2274A5", 0.2))) + 
  geom_raster(data = densdf_hh, aes(x = x, y = y, fill = z), interpolate = TRUE) +
  scale_fill_gradientn(colours = c(scales::alpha("#D71515", 0),
                                   scales::alpha("#D71515", 0.2))) + 
  geom_raster(data = densdf_hd, aes(x = x, y = y, fill = z), interpolate = TRUE) +
  scale_fill_gradientn(colours = c(scales::alpha("#650304", 0),
                                   scales::alpha("#650304", 0.2)))
ggplot() +
  # Raster Cold Healthy
  geom_raster(data = densdf_ch, aes(x = x, y = y, alpha = z), 
              fill = "#2274A5", interpolate = TRUE) +
  
  # Raster Hot Healthy
  geom_raster(data = densdf_hh, aes(x = x, y = y, alpha = z), 
              fill = "#D71515", interpolate = TRUE) +
  
  # Raster Hot Damaged
  geom_raster(data = densdf_hd, aes(x = x, y = y, alpha = z), 
              fill = "#650304", interpolate = TRUE) +
  
  scale_alpha(range = c(0, 0.3), guide = "none") +  # Controla transparencia sin leyenda
  theme_classic() + 
