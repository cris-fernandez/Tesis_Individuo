...
clean_target <- clean_target %>% 
  ...
  dplyr::select(c(mean_def_obs, height, dbh, total_chl_fw_22, xc_fw_22, 
                  ...
                  sp_id, wc_22, category)) %>% 
  dplyr::select(sort(names(.))) %>% 
  relocate(sp_id, category)
...
traits_df <- clean_target %>% 
  dplyr::select(-c(wood_d13c_17, wood_d13c_22))

traits_wilcox <- list()

for (i in 1:(ncol(traits_df)-2)) { # Because category and tree_number are not numeric
  traits_wilcox[[i]] <- pairwise.wilcox.test(traits_df[, i+2], traits_df$category,
                                           p.adjust.method = "bonferroni")
  print(i)
}

# 7.- Grouping results in a single df ####

# We need to create vectors with the name of the variable analyzed, which 
# is each name of the original vuln_ and resp_df

name_vars <- names(traits_df)[3:ncol(traits_df)]
...
traits_df_long <- convert_pw_wilcox(traits_wilcox, name_vars, "var")

# Column telling whether it is significant or not!
traits_df_long$significant <- ifelse(traits_df_long$P_value_adjusted < 0.05, 1, 0)

# Summarising by variable, to get those with higher amount of significant differences
sig_count <- traits_df_long %>% 
  group_by(Variable) %>% 
  summarise(significant_difs = sum(significant, na.rm = T))

sig_mean <- traits_df_long %>% 
  group_by(Variable) %>% 
  summarise(pval_mean = mean(P_value_adjusted, na.rm = T))

# Now just comparing pairs of hot vs. coldspot per species:
status_pairs <- traits_df_long %>% 
  mutate(grupo1 = gsub("_hotspot|_coldspot", "", Group1),
         grupo2 = gsub("_hotspot|_coldspot", "", Group2),
